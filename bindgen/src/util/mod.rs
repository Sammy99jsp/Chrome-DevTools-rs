...
pub trait Contextual {
    fn next(self, ident: impl ToString) -> Option<Context>;
...
impl Contextual for Option<Context> {
    ...
    fn next(self, ident: impl ToString) -> Option<Context> {
        ...
        match self {
            None => Some(Protocol),
            Some(Protocol) => Some(Domain(ident.to_string())),
            Some(Domain(p1)) => Some(Item(p1, ident.to_string())),
            Some(Item(p1, p2)) => Some(Field(p1, p2, ident.to_string())),
...
pub mod serde {
    ...
    use crate::protocol::{convention as conv, modular};
    ...
    impl<NC: conv::NamingConvention> GenRenameMacro for modular::Identifier<NC> {
        fn serde_rename(&self, span: Span) -> [syn::Attribute; 1] {
